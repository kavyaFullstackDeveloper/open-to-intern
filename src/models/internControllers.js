...
try {
let data = req.body
let name = await collegeModel.findOne({ name: data.collegeName }).select({ collegeName: 1 })
...
if (!name) return res.status(400).send({ status: false, message: "Must enter college name to find" })
if (name.length == 0) {
